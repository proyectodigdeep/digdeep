angular.module('digdeepApp.profilesLSCtrl', [])

.config(['$stateProvider', function ($stateProvider) {
	$stateProvider.state('profilesLS', {
		url: '/profilesLS',
		templateUrl: '/app/templates/profilesLS.html',
		controller: 'profilesLSCtrl',
		params: {
        	idSubcat: "",
        	titlesubcat: ""
    	}
...
.controller('profilesLSCtrl', [ '$scope', '$state', '$rootScope', 'serviceService', '$localStorage','categoryService', '$interval','userService',
function (                  	 $scope,   $state,   $rootScope,   serviceService,   $localStorage,  categoryService,   $interval,  userService) {
	...
	$scope.goDigdeepersLs = function (idSubcategory, titleSubcat) {
    ...
    $scope.goServicesToDigdeeper = function (idDigdeeper) {
        $state.go("servicesLS",{idDigdeeper: idDigdeeper})
    }
}])
