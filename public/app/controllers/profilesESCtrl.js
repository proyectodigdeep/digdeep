angular.module('digdeepApp.profilesESCtrl', [])

.config(['$stateProvider', function ($stateProvider) {
	$stateProvider.state('profilesES', {
		url: '/profilesES',
		templateUrl: '/app/templates/profilesES.html',
		controller: 'profilesESCtrl',
		params: {
        	idSubcat: "",
        	titlesubcat: ""
    	}
...
.controller('profilesESCtrl', [ '$scope', '$state', '$rootScope', 'serviceService', '$localStorage','categoryService', '$interval','userService',
function (                  	 $scope,   $state,   $rootScope,   serviceService,   $localStorage,  categoryService,   $interval,  userService) {
	...
	$scope.goDigdeepersEs = function (idSubcategory, titleSubcat) {
    ...
    $scope.goServicesToDigdeeper = function (idDigdeeper) {
        $state.go("servicesES",{idDigdeeper: idDigdeeper})
    }
}])
