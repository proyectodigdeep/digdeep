angular.module('digdeepApp.profilesHOCtrl', [])

.config(['$stateProvider', function ($stateProvider) {
	$stateProvider.state('profilesHO', {
		url: '/profilesHO',
		templateUrl: '/app/templates/profilesHO.html',
		controller: 'profilesHOCtrl',
		params: {
        	idSubcat: "",
        	titlesubcat: ""
    	}
...
.controller('profilesHOCtrl', [ '$scope', '$state', '$rootScope', 'serviceService', '$localStorage','categoryService', '$interval','userService',
function (                  	 $scope,   $state,   $rootScope,   serviceService,   $localStorage,  categoryService,   $interval,  userService) {
	...
	$scope.goDigdeepersHO = function (idSubcategory, titleSubcat) {
    ...
    $scope.goServicesToDigdeeper = function (idDigdeeper) {
        $state.go("servicesHO",{idDigdeeper: idDigdeeper})
    }
}])
